...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5 class = "fst-italic">Reviews : <span class="text-danger">${product.rating.count}</span></h5>
      <h5 class = "fst-italic">Rating: <span class="text-danger ">${product.rating.rate}</span></h5>
      </div>
      
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success p-3 rounded-3 mx-4">Add to cart</button>
      <button type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="showDetails(${product.id})" id="details-btn" class="btn btn-danger p-3 rounded-3">Details</button>
